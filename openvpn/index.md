...
進入 VPN 伺服器, 他可以建立兩個 OpenVPN server, 預設是 Server 1, 把它開啟. 我是只有要透過 VPN 來連內網, 所以設定 "LAN only". 如果需要使用者輸入帳號密碼, 請在最後面新增一個 username.
...
要注意的是, Asuswrt-Merlin 產生的 client1.ovpn 裡面給的是 IP, 而如果你用的不是固定 IP, 而是使用 DDNS 之類的, 請記得修改裡面 remote 後面的 IP. 下面是 DDNS 的設定
